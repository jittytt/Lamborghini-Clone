                ...
                if (docm.exists()) {
                    ...
                    getDoc(userDataDocRef)
                        ...
                        if (userDoc.exists()) {
                                ...
                                cartItemDiv.innerHTML = `
                                    ...
                                    <div class="cart-item-info">
                                        ...
                                        <div class="cart-product-price">
                                            &#36; <span class="cart-product-price-value">${item.price.toFixed(2)}</span>
                                        </div>
                                        <div class="cart-product-quantity">
                                            <svg class="minus-icon" focusable="false" viewBox="0 0 24 24" aria-hidden="true" onclick="decrementProductQuantityPopupNav('${item.product_id}', '${item.size}')"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11H7v-2h10v2z"></path></svg>
                                            <span class="cart-product-quantity-number">${item.count}</span>
                                            <svg class="plus-icon" focusable="false" viewBox="0 0 24 24" aria-hidden="true" onclick="incrementProductQuantityPopupNav('${item.product_id}', '${item.size}')"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11h-4v4h-2v-4H7v-2h4V7h2v4h4v2z"></path></svg>                            
                            ...
                            const viewCartDiv = document.querySelector('.view-cart-div');
                            viewCartDiv.innerHTML = `
                            <!-- Add this onclick attribute to your button -->
                            <button CLASS='view-cart-button' onclick="openViewCartPage()">VIEW CART</button>
                            `;
                          } else {
